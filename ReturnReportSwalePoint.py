...
def returnInventory_and_Inspection_Information(OBJECTID):
    try:
        ...
        sql = ("select sw.CONNECTION_TYPE,sw.IS_OUTLET,sw.DISCHARGE_DSTRM_TYPE,sw.OWNERSHIP_DOWNSTRM, (case when insp.INSPECTION_DATE is null then '' else FORMAT(insp.INSPECTION_DATE, 'M/d/yyyy' ) end) as d " +
                "from deldot_migration2.dbo.STRUCTURES_evw  s " +
                "join deldot_migration2.dbo.SWALE_CONNECTIONS_evw sw on sw.STRUCTURE_ID=s.GlobalID " +
                "left join deldot_migration2.dbo.SWALE_CONN_INSPECTIONS_evw insp on insp.SWALE_CONNECTION_ID=sw.GlobalID where s.OBJECTID = " + OBJECTID + " order by insp.INSPECTION_DATE desc ")     
 
        result = ReportSharedFunctions.returnDataODBC(sql)[0]
        result = ['' if x == None else x for x in result] 

        CONNECTION_TYPE = result[0]
        CONNECTION_TYPE = ReportSharedFunctions.valFromDomCode("D_SwaleConn_Type", CONNECTION_TYPE)

        IS_OUTLET = result[1]
        if IS_OUTLET != "": IS_OUTLET = ReportSharedFunctions.boolVal(IS_OUTLET)

        DISCHARGE_DSTRM_TYPE = result[2]
        DISCHARGE_DSTRM_TYPE = ReportSharedFunctions.valFromDomCode("D_Discharge_Dstrm_Type", DISCHARGE_DSTRM_TYPE)

        OWNERSHIP_DOWNSTRM = result[3]
        OWNERSHIP_DOWNSTRM = ReportSharedFunctions.valFromDomCode("D_Ownership", OWNERSHIP_DOWNSTRM)

        Last_Inspected_Date = result[4]   

        content = [['Swale Point Type:',CONNECTION_TYPE]]
        content.append(['Is Outlet:',IS_OUTLET])
        content.append(['Discharge Downstream Type:',DISCHARGE_DSTRM_TYPE])
        content.append(['Ownership Downstream:',OWNERSHIP_DOWNSTRM]) #move to prod
...
def returnOutfall_Information(OBJECTID):
    try:
        sql = ("SELECT top 1 ofs.NPDES,ofs.STREAM_DISTANCE,ofs.GNIS_NAME,ofs.GNIS_ID,oda.DRAINAGE_AREA_AC,insp.OVERALL_RATING,ofs.OF_PROTECTION_MATERIAL,"+
               ...
               "join  [deldot_migration2].[dbo].SWALE_CONNECTIONS_evw pe on pe.STRUCTURE_ID = s.[GlobalID]  " +
            "left join  [deldot_migration2].[dbo].SWALE_CONN_INSPECTIONS_evw pei on pei.SWALE_CONNECTION_ID = pe.[GlobalID]  "+ 
